...
async function findDistinctValues(dataset, colName) {
  const sql = `
    SELECT DISTINCT \`${colName}\` FROM \`${dataset.dataTable}\`
    ORDER BY \`${colName}\`
  `;
...
async function findValuesByColInfo(dataset, targetColName, colName, colValue) {
  const sql = `
    SELECT \`${targetColName}\` FROM \`${dataset.dataTable}\`
    WHERE \`${colName}\` = '${colValue}'
    ORDER BY \`${targetColName}\`
  `;
