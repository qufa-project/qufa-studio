...
function extractMeta(records, skip) {
  ...
  for (let i = 0; i < headers.length; i++) {
    const newMeta = exteactResultToMeta(result, i);
    newMeta.koName = headers[i];
    newMeta.name = headers[i].cleanup();
    if (!colMap[newMeta.name]) {
      colMap[newMeta.name] = 1;
    } else {
      const colIdx = colMap[newMeta.name];
      colMap[newMeta.name] = colMap[newMeta.name] + 1;
      newMeta.name = `${newMeta.name}_${colIdx}`;
    }
    metas.push(newMeta);
  }
 ...
 *
 * @param {Object} result
 * @param {string[]} result.types AcceptableType 배열
 * @param {number[]} result.lengths types가 varchar 또는 text인 경우 동일한 index의 length를 사용
 * @param {number[]} result.maxValues types가 int 또는 bigint인 경우 동일한 index의 최대값 사용
 * @param {Object[]} result.doubleLength types가 Decimal인 경우 동일한 index의 length값 사용 (m+d, d)
 * @param {number[]} result.doubleLength.m 정수부 길이
 * @param {number[]} result.doubleLength.d 길수부 길이
 */
function exteactResultToMeta(result, i) {
  const newMeta = new Meta();
  let colType;
  let maxLength;
  let floatLength;

  colType = result.types[i];
  maxLength = result.lengths[i];

  if (colType === AcceptableType.DECIMAL) {
    maxLength = result.doubleLength[i].m + result.doubleLength[i].d;
    floatLength = result.doubleLength[i].d;
  }

  switch (colType) {
    case AcceptableType.VARCHAR:
    case AcceptableType.TEXT:
      newMeta.colType = "string";
      break;
    case AcceptableType.INTEGER:
    case AcceptableType.BIGINT:
    case AcceptableType.DECIMAL:
      newMeta.colType = "number";
      break;
    default:
      newMeta.colType = "string";
      break;
  ...
  newMeta.maxLength = maxLength;
  if (floatLength) {
    newMeta.floatLength = floatLength;
  }
  ...
  for (let i = skip; i < records.length; i++) {
    if (i === skip) {
      for (let record of records[i]) {
        ...
        doubleLength.push(
          type === AcceptableType.DECIMAL ? countDecimalMD(record) : null
        );
      }
      continue;
    }
    ...
    for (let j = 0; j < records[i].length; j++) {
      ...
      if (types[j] === AcceptableType.INTEGER) {
        maxValues[j] =
          maxValues[j] < Number(currentRecord)
      ...
      if (
        availableType !== types[j] ||
        (availableType === AcceptableType.DECIMAL && availableType === types[j])
      ) {
        if (
          (availableType === AcceptableType.DECIMAL &&
            types[j] === AcceptableType.INTEGER) ||
          (availableType === AcceptableType.INTEGER &&
          ...
          doubleLength[j].m =
            decimalMD.m > doubleLength[j].m ? decimalMD.m : doubleLength[j].m;
          doubleLength[j].d =
            decimalMD.d > doubleLength[j].d ? decimalMD.d : doubleLength[j].d;
        } else if (
          types[j] === null &&
          availableType === AcceptableType.INTEGER
        ) {
